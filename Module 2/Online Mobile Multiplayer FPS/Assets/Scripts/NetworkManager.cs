...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        connectionStatusText.text = "Connection status " + PhotonNetwork.NetworkClientState; // provides info to how you're
                                                                                             // connected to Photon Servers                                                                                       
    }
    #endregion
    ...
    {
        if (!PhotonNetwork.InLobby) // checks if we're in the lobby or not
        {
    ...
    public override void OnConnected() // Called when connected to the internet 
    {
        Debug.Log("Connected to the Internet");
    }

    public override void OnConnectedToMaster() // Called when connected to Photon Servers (successfully logged in)
    {
    ...
    public override void OnJoinedRoom()
    {
        ...
        foreach(Player player in PhotonNetwork.PlayerList /* --> contains players inside a room*/)
        {
            ...
            playerItem.transform.Find("PlayerIndicator").gameObject.SetActive
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList) // Returns a list of all of the room
                                                                   // information inside a lobby      
    {
        ...
        foreach(RoomInfo info in roomList)
        {
            ...
            if (!info.IsOpen || !info.IsVisible || info.RemovedFromList) // if room is already added into the dictionary
            {
    ...
    {
        foreach(var /*GameObject*/ item in roomListGameObjects.Values)
        {
            Destroy(item);
        }
